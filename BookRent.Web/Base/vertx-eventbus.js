...
!function (factory) {
  if (typeof require === 'function' && typeof module !== 'undefined') {
    ...
    WebSocketEventBus = factory(this.SockJS);
  }
}(function (SockJS) {
   ...
   * @constructor
   */
  var WebSocketEventBus = function (url, options) {
    var self = this;
    ...
    this.state = WebSocketEventBus.CONNECTING;
    this.handlers = {};
    this.replyHandlers = {};
    ...
    this.sockJSConn.onopen = function () {
      ...
      self.state = WebSocketEventBus.OPEN;
      self.onopen && self.onopen();
    };

    this.sockJSConn.onclose = function () {
      self.state = WebSocketEventBus.CLOSED;
   ...
   */
  WebSocketEventBus.prototype.send = function (address, message, headers, callback) {
    // are we ready?
    if (this.state != WebSocketEventBus.OPEN) {
   ...
   * @param {Object} [headers]
   */
  WebSocketEventBus.prototype.publish = function (address, message, headers) {
    // are we ready?
    if (this.state != WebSocketEventBus.OPEN) {
      throw new Error('INVALID_STATE_ERR');
    }
   ...
   * @param {Function} callback
   */
  WebSocketEventBus.prototype.registerHandler = function (address, headers, callback) {
    // are we ready?
    if (this.state != WebSocketEventBus.OPEN) {
      throw new Error('INVALID_STATE_ERR');
    }
   ...
   * @param {Function} callback
   */
  WebSocketEventBus.prototype.unregisterHandler = function (address, headers, callback) {
    // are we ready?
    if (this.state != WebSocketEventBus.OPEN) {
      throw new Error('INVALID_STATE_ERR');
    }
   ...
   */
  WebSocketEventBus.prototype.close = function () {
    this.state = WebSocketEventBus.CLOSING;
    this.sockJSConn.close();
  };

    WebSocketEventBus.prototype.reconnect = function () {
        this.state = WebSocketEventBus.CLOSING;
        this.sockJSConn.close();
        sendPing();
        pingTimerID = setInterval(sendPing, pingInterval);
  ...
  WebSocketEventBus.CONNECTING = 0;
  WebSocketEventBus.OPEN = 1;
  WebSocketEventBus.CLOSING = 2;
  WebSocketEventBus.CLOSED = 3;

  if (typeof exports !== 'undefined') {
    if (typeof module !== 'undefined' && module.exports) {
      exports = module.exports = WebSocketEventBus;
    } else {
